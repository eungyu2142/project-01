...
const links = [
  ...
  { to: '/mypets', label: '반려동물 관리', icon: 'pets' as const },
...
export function BottomNav() {
  return (
    <div className="app-nav-wrap pointer-events-none absolute inset-x-0 bottom-0 z-40 flex justify-center px-0">
      <nav className="app-nav pointer-events-auto grid h-[5.5rem] w-full grid-cols-4 border-t border-emerald-100/80 bg-[#d7f5e7]/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-3 shadow-[0_-12px_30px_rgba(15,118,110,0.10)] backdrop-blur sm:rounded-b-[2.5rem]">
        {links.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
