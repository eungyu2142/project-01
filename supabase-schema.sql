...
alter table speech_summary_jobs enable row level security;
...
drop policy if exists "Users can read own speech summary jobs" on speech_summary_jobs;
...
on speech_summary_jobs
for select
to authenticated
using ((select auth.uid()::text) = user_id);

grant select on speech_summary_jobs to authenticated;
